# src/osm_features.py
# Helper functions for Notebook 04: pull OSM points of interest and count
# how many fall within a buffer distance of each hexagon.
...
def fetch_pois(boundary_geom, tags):
    """Download OSM features matching `tags` inside `boundary_geom`.

    boundary_geom: a shapely geometry in EPSG:4326 (lat/lon) - same as
    berlin_boundary from notebook 01.
    tags: an OSM tag dict, e.g. {"shop": True} or {"amenity": "restaurant"}.

    OSM features can be points, lines, or polygons (a bus stop is a point,
    a park is a polygon). We convert everything to a single representative
    point (its centroid) so every feature type can be counted the same way.
    """
...
def count_in_buffer(grid, pois, buffer_m, col_name):
    ...
    Uses centroid-based buffering, consistent with the centroid-containment
    approach already used for grid generation (notebook 01) and the target
    variable (notebook 03).
    """
